...
import { claimOpenShift, listOpenShifts } from "../api/shifts";
...
export default function OpenShiftsScreen() {
  ...
  const load = useCallback(async () => {
    setRefreshing(true);
    try {
      const data = await listOpenShifts();
      setShifts(data);
    } finally {
      setRefreshing(false);
    }
  ...
  return (
    <FlatList
      ...
      ListHeaderComponent={<Text style={styles.title}>Open Shifts</Text>}
      renderItem={({ item }) => (
        <View style={styles.card}>
          <Text style={styles.main}>{item.date} • {item.startTime} - {item.endTime}</Text>
          <Pressable style={styles.button} onPress={() => claimOpenShift(item._id).then(load)}>
            <Text style={styles.buttonText}>Claim Shift</Text>
          </Pressable>
        </View>
      )}
      ListEmptyComponent={<Text>No open shifts.</Text>}
    />
  );
}
