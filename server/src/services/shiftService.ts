...
import Shift, { IShift, ShiftStatus } from "../models/Shift.js";
...
async function validateShiftAssignmentRules(input: {
  ...
  if (availability) {
    ...
    const allowed = dayRanges.some((range) =>
      toMinutes(range.startTime) <= toMinutes(input.startTime) &&
...
export async function updateShift(
  ...
  performedBy: string,
  companyId: string,
  payload: Partial<Pick<IShift, "employeeId" | "date" | "startTime" | "endTime" | "status">>
) {
  ...
  if (payload.date) shift.date = payload.date;
  ...
  if (payload.employeeId !== undefined) {
    shift.employeeId = payload.employeeId ? (payload.employeeId as never) : null;
  }
...
export async function listShifts(companyId: string, options?: { mineOnly?: string }) {
...
export async function listOpenShifts(companyId: string) {
...
export async function generateSchedule(input: {
  ...
  const availabilityByUser = new Map(
    availabilityDocs.map((doc) => [doc.userId.toString(), doc])
  );
...
export async function releaseShiftEmergency(
  shiftId: string,
  requesterId: string,
  companyId: string
) {
...
export async function claimOpenShift(
  ...
  const previousData = shift.toObject();
  shift.employeeId = employeeId as never;
