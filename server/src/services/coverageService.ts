...
export async function listCoverageRequestsForUser(userId: string) {
...
export async function respondCoverageRequest(input: {
  ...
  if (input.action === "declined") {
    ...
    await claimOpenShift(request.shiftId.toString(), input.targetEmployeeId, input.companyId).catch(
      async () => {
        const shift = await Shift.findById(request.shiftId);
        if (!shift) throw new Error("Shift not found");
        shift.employeeId = input.targetEmployeeId as never;
