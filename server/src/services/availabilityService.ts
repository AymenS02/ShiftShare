import AvailabilityPreference, {
  ITimeRange,
} from "../models/AvailabilityPreference.js";
import User from "../models/User.js";
import { createAuditLog } from "./auditService.js";

type WeeklyAvailabilityInput = Record<string, ITimeRange[]>;
...
function validateRange(range: ITimeRange) {
...
function validateNoOverlap(ranges: ITimeRange[]) {
...
export async function upsertAvailabilityPreference(
  userId: string,
...
export async function getAvailabilityPreference(userId: string) {
  const user = await User.findById(userId).select("companyId");
  if (!user || !user.companyId) {
    throw new Error("User must belong to a company");
  }
