import Company, { SchedulingSettings } from "../models/Company.js";
...
export async function createCompany(
  ...
  manager.companyId = company._id as never;
  await manager.save();
...
export async function joinCompanyByCode(userId: string, companyCode: string) {
  ...
  if (!company.employees.some((id) => id.toString() === userId)) {
    company.employees.push(user._id as never);
    await company.save();
  }
...
export async function updateCompany(
  managerId: string,
  companyId: string,
  payload: Partial<{ companyName: string; companyCode: string }>
) {
